          ...
          <!-- Modal -->
                  ...
                  <td data-label="Produse" style="overflow: auto;">
                    ...
                    <!-- Modal -->
                            ...
                            <td data-label="Status comanda" >
                              @if ($order->order_status == 0)
                                <div style="color:yellow">In desfasurare</div>
                                <form
                                method="POST"
                                ...
                                @csrf
                                @method('PUT')
                                <button type="submit" class="bxtn bxtn3" style="font-size: 15px;font-family: sans-serif; background-color:#0d6efd">Change to:<br> Completa</button>
                                </form>
                              @else
                                <div style="color:yellowgreen">Completa</div>
                                <form
                                method="POST"
                                action="{{ route('orders.update', $order->id) }}"
                                onsubmit="return confirm('Are you sure?');">
                                @csrf
                                @method('PUT')
                                <button type="submit" class="bxtn bxtn3" style="font-size: 15px;font-family: sans-serif; background-color:#0d6efd">Change to:<br> In desfasurare</button>
                                </form>
                              @endif
                            </td>
                            @endforeach
